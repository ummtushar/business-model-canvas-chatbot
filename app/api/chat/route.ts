...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'load') {
      return handleLoadChat(chatId);
    }

    if (action === 'save') {
      return handleSaveChat(chatId, messages);
    }
    ...
    return handleModelRequest(model, messages);
  } catch (error) {
...
function handleLoadChat(chatId: string | undefined) {
  if (!chatId) {
...
function handleSaveChat(chatId: string | undefined, messages: Message[] | undefined) {
  if (!chatId || !messages) {
...
function handleDeleteChat(chatId: string | undefined) {
  if (!chatId) {
    return NextResponse.json({ success: false, message: 'Chat ID is missing' }, { status: 400 });
  }
  delete chats[chatId];
...
async function handleModelRequest(model: Model, messages: Message[]) {
